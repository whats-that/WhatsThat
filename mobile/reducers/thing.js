...
export const fetchThings = () => async dispatch => {
  try {
    const res = await axios.get('http://192.168.0.101:8080/api/thing');
...
export const fetchUserThing = userId => async dispatch => {
  try {
    ...
    const res = await axios.get(
      `http://192.168.0.101:8080/api/thing/${userId}`
    );
    const things = res.data;
...
export const createThing = thing => async dispatch => {
  try {
    const res = await axios.post(`http://192.168.0.101:8080/api/thing`, thing);
